        ...
        try { //Checks to see the command if valid
            ...
            if (message.get(1).equals("list")) { //Shows the list of events
                ...
                if (ProfileHandler.areEvents(id)) {
                    ...
                    SoapHandler.sendTextMessageInChannel("There are no events currently active", event.getMessage().getChannel().block());
                }
                    ...
                    for (ReserveEvent reserve: ProfileHandler.getEvents(id)) {
                        if (reserve.getIdentifier().equals(message.get(2))) {
                            ...
                            if (reserve.getReserved() > 0) {
                                ...
                                SoapHandler.sendTextMessageInChannel("You have unreserved from event " + reserve.getIdentifier(), event.getMessage().getChannel().block());
                            } else {
                                SoapHandler.sendTextMessageInChannel("There are no more people reserved to this event, this event has been removed", event.getMessage().getChannel().block());
                            }
                        }
                    }
                } else {
                    SoapHandler.sendTextMessageInChannel("Event does not exist", event.getMessage().getChannel().block());
                }
                ...
                if (ProfileHandler.eventExists(id, message.get(1))) {
                    ...
                    if (reserve.isTimeless()) {
                        ...
                        response.append("\t- Time: " + SoapHandler.convertToAmPm(reserveObject.get("time").getAsString()) + "\n");
                        response.append("This event will pop at " + SoapHandler.convertToAmPm(reserve.getTime()));
                    }
                    response.append("\nReserved users:\n");
                    for (String user : reserve.getReservedUsers()) {
                        Member member = SoapHandler.memberMatcher(user, event.getGuild().block().getMembers().buffer().blockFirst());
                        response.append("\t- " + member.getUsername() + "\n");
                    }
                    SoapHandler.sendTextMessageInChannel(response.toString(), event.getMessage().getChannel().block());
                } else {
                    SoapHandler.sendTextMessageInChannel("This event does not exist, type !events list for a list of all active events", event.getMessage().getChannel().block());
                }
            }
        } catch (IndexOutOfBoundsException e) {
            SoapHandler.sendTextMessageInChannel(help(), event.getMessage().getChannel().block());
        }
    }
