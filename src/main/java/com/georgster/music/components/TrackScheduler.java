...
public final class TrackScheduler extends AudioEventAdapter implements AudioLoadResultHandler {
    ...
    public void trackLoaded(final AudioTrack track) {
        try {
            ...
            if (queue.size() == 1) {
                ...
                ThreadPoolFactory.scheduleVoiceTask(handler.getId(), () -> handler.sendText("Queued up track: " + track.getInfo().title));
            }
    ...
    public void playlistLoaded(final AudioPlaylist playlist) {
        ...
        ThreadPoolFactory.scheduleVoiceTask(handler.getId(), () -> handler.sendText("Queued up track: " + handler.sendText("Queued up playlist: " + playlist.getName())));
    ...
    public void onTrackStart(AudioPlayer player, AudioTrack track) {
        ThreadPoolFactory.scheduleVoiceTask(handler.getId(), () -> handler.sendText("Now Playing: " + track.getInfo().title)); //The queuing and playing happens in onTrackLoaded, so we will just send a message here.
    }
    ...
    public void noMatches() {
        ThreadPoolFactory.scheduleVoiceTask(handler.getId(), () -> {
            handler.sendText("Failed to grab audio from the provided arguments");
            if (!isActive()) {
    ...
    public void loadFailed(final FriendlyException exception) {
        ThreadPoolFactory.scheduleVoiceTask(handler.getId(), () -> {
            handler.sendText("Failed to load track: " + exception.getMessage());
            if (!isActive()) {
                connection.disconnect().block(); //If the queue is empty, we will disconnect from the voice channel.
            }
        });
